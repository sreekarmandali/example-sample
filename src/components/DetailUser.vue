            ...
            <tbody>
              <tr v-for="(element,index) in users">
                ...
                <td>
                  ...
                  </b-button><b-button class="btn btn-default border" id="deleteButton" @click="showMsgBoxOne(index)">
...
<script lang="js">
  ...
  export default {
    ...
    methods: {
      ...mapMutations(["ADD_USER", "DELETE_USER"]),
      ...mapActions(["loadUsers","deleteUsers","addUsers"]),
      addUser() {
        console.log("hello");
        this.addUsers({ "name": this.name, "location": this.location, "itemcount": this.itemcount, "bill": this.bill,"id":3 });
        this.name = '';
        this.location = '';
          this.itemcount = '';
          this.bill = '';
